...
public class Panneau_jeu extends JPanel {
	Image carotte, clef, cuisinier, infirmiere, lapin, potion, renard, terrier, tresor; 	//Différentes images à afficher
	...
	//Dessin de ce qu'il faudra afficher dans la fenêtre de jeu principale
	public void paintComponent(Graphics g) {
		...
		for(int i=0; i<Scenario.cloisonsH.length; i++) {
			for(int j=0; j<Scenario.cloisonsH[0].length; j++) {
				if(i<Scenario.cloisonsV.length && !Scenario.pieces[i][j].visitee) {		//On dessine en gris les pièces qui n'ont pas été visitées
					g.setColor(new Color(128,128,128));
					g.fillRect((j*w/10)+1, (i*h/7)+1, (w/10), (h/7));
					g.setColor(Color.black);
				}
				//Dessin des cloisons horizontales autour des pièces visistées
				if(Scenario.cloisonsH[i][j].getClass().getSimpleName().equals("Porte") && (Scenario.pieces[i][j].visitee || Scenario.pieces[i-1][j].visitee)) {
					if(Scenario.cloisonsH[i][j].locked==true) {		//Si la porte est fermée, trait noir coupé par un trait rouge symbolisant la porte fermée
					...
					else {		//Si la porte est ouverte, trait noir coupé par un trait blanc symbolisant l'ouverture de la porte
						g.fillRect((j*w/10), (i*h/7)-1, (w/30), 2);
			...
			if(i<Scenario.cloisonsV.length) {
				for(int j=0; j<Scenario.cloisonsV[0].length; j++) {
					//Dessin des cloisons verticales autour des pièces visistées
					if(Scenario.cloisonsV[i][j].getClass().getSimpleName().equals("Porte") && (Scenario.pieces[i][j].visitee || Scenario.pieces[i][j-1].visitee)) {
						if(Scenario.cloisonsV[i][j].locked==true) {		//Si la porte est fermée, trait noir coupé par un trait rouge symbolisant la porte fermée
					...
					//Uniquement dans les pièces visitées
